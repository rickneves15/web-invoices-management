...
import { getEnergyStats } from '~/services/metrics'
...
export const EnergyStatsChart = () => {
  ...
  const selectedCustomer =
    searchParams.get('customer-stats') || DEFAULT_SELECT_OPTION.value
  ...
  const handleResetCustomer = () => {
    setQueryParams('customer-stats', DEFAULT_SELECT_OPTION.value)
  }

  const handleChange = (value: string) => {
    setQueryParams('customer-stats', value)
  }
          ...
          <>
            {energyStatsData.length > 0 ? (
              <ResponsiveContainer width="100%" height={240}>
                <LineChart data={energyStatsData} style={{ fontSize: 12 }}>
                  <XAxis
